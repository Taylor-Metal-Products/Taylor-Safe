...
GitHub permission, Supabase platform permission, and Taylor Safe app membership are separate. Wes's requested Taylor Safe app account is **not provisioned by this handoff**. The proposed [team-access flow](supabase-change-requests/2026-09-18-team-access-invitations.md) keeps Auth account creation manual with Louie, then lets an app administrator copy a company invitation for the recipient to accept after signing in. It requires the reviewed backend migration and frontend release; it is not yet applied or deployed. Do not invoke the first-owner/new-company bootstrap to add someone to the existing company. Manual account creation does not solve recovery-email delivery, grant platform access, or automatically create company membership.
