...
|---|---:|---|---|
| Full `npm test` Playwright run, desktop and Pixel 7 projects | PASS: 106 passed, 8 skipped, 0 failed | In addition to the prior auth, tenant, responsive, inspection, incident, program, and operational-workflow coverage, the harness passes the Oregon manufacturing-priority guide, metal-fabrication term search, exclusion of construction rules from the default view, full Oregon catalog access, state-authority switching, all-location fail-closed behavior, jurisdiction-labeled global state search, source trace preservation, and mobile overflow checks. | Real authenticated workflow persistence, qualified applicability review, source-byte ingestion, scanner behavior, hosted callback exchange and email delivery, real devices, accessibility, or weak networks. |
...
| Skip review | ACCEPTED FOR THIS LAYER | Eight skip instances comprise two tests requiring ignored private fixture/original PDFs in both projects, three desktop skips of mobile-only overflow checks, and one mobile skip for global search, which is intentionally hidden in the compact mobile shell. They are conditional/project skips, not failures. | The unavailable private fixture/original PDF paths and global-search behavior on the compact mobile shell. |
...
| Public regulatory source evaluation | PASS | Public tenant/program collections are empty; tenant control links are absent; the federal artifact has 1,547 structural records current through 2026-07-29; the state artifact has 44 curated records (24 OR, including 23 manufacturing-focused priorities; 10 WA; 10 CA). All 20 unique Oregon official destinations returned HTTP 200 on 2026-08-06; the Washington and California starter-link check date remains 2026-07-30. | Complete state coverage, state source-byte hashes, legal applicability, semantic confirmation that every current page is unchanged, or current official text after the artifact date. |
